intron_filter_by_GRanges <- function(gr_input){
  ...
  if(length(recombined_gr_base) == 1){gr_combined = recombined_gr_base}
  ...
  list("prefiltered_new_exon" = isoform_names[!pre_test], "remain" = isoform_names_filtered[test], "filtered" = isoform_names_filtered[test==F],
       "prefiltered_binary" = out_disjoin[, !pre_test], "remain_binary" = out_disjoin_filtered[, test], "filtered_binary" = out_disjoin_filtered[, !test],
