intron_filter <- function(gff){
  ...
  require("stringr"); require("rtracklayer"); require("GenomicRanges"); #require("pbapply")
  ...
  if(!file.exists(gff)){print("No File.")}
  
  ##Prepare gtf file from hg38
  print(paste(Sys.time(), ": preparing gtf file from hg38")); flush.console()
  if(!file.exists("Gencode_proteinCoding.gtf") & file.exists(gff)){
  ...
  print(paste(Sys.time(), ": aligning sequence")); flush.console()
  ...
  if(sum(!pre_test)>0){export(gr[gr$transcript_id %in% isoform_names[!pre_test]], paste(split_name[1],"_potential_new_exon",".",split_name[2], sep=""))}
  if(sum(test)>0){export(gr[gr$transcript_id %in% isoform_names_filtered[test]], paste(split_name[1],"_remain2.0",".",split_name[2], sep=""))}
  if(sum(!test)>0){export(gr[gr$transcript_id %in% isoform_names_filtered[test==F]], paste(split_name[1],"_filtered2.0",".",split_name[2], sep=""))}
